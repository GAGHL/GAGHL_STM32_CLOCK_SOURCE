...
void System_Check_Clock_Source(void) {
	...
	switch (clock_source) {
		case 0x00:
			current_clock_source = System_Clock_is_HSI;
			break;

		case 0x01:
			current_clock_source = System_Clock_is_HSE;
			break;

		case 0x02:
			current_clock_source = System_Clock_is_PLL;
			break;
	}
}
...
void PLL_Check_Clock_Source(void) {
	...
	if (((RCC->CFGR >> 2) & 0x3) == (0x02)) {
		if (pll_source == 0x01) {
			...
			current_pll_clock_source = PLL_Source_is_HSI_div_2;
		}
	} else {
